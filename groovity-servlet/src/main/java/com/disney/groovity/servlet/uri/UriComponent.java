 ...
 * holder.
 */
package com.disney.groovity.servlet.uri;
...
public class UriComponent {
    ...
    public static void validate(final String s, final Type t, final boolean template) {
        ...
        if (i > -1) {
            throw new IllegalArgumentException(String.format("The string \"%s\" for the URI component %s contains an invalid character, ''%s'', at index %d.",s, t, s.charAt(i), i));
        }
    }
    ...
    public static String decode(final String s, final Type t) {
        ...
        if (s.indexOf('%') < 0) {
            ...
            if (n < 2) {
                throw new IllegalArgumentException(String.format("Malformed percent-encoded octet at index %d.", 1));
            }

            // Malformed percent-escaped octet at the end
            if (s.charAt(n - 2) == '%') {
                throw new IllegalArgumentException(String.format("Malformed percent-encoded octet at index %d.",n - 2));
            }
        }
    ...
    private static int decodeHex(final String s, final int i) {
        ...
        if (v == -1) {
            throw new IllegalArgumentException(String.format("Malformed percent-encoded octet at index %d, invalid hexadecimal digit ''%dx''.",i, s.charAt(i)));
        }
        return v;
    }
