...
public class UriTemplateParser {
    ...
    private void parse(final CharacterIterator ci) {
        try {
            ...
            throw new IllegalArgumentException(
            		String.format(
            				"Invalid syntax in the template \"%s\". Check if a path parameter is terminated with a \"}\".",
            				template), 
            ex);
        }
    }
    ...
    private int parseName(final CharacterIterator ci, int skipGroup) {
        ...
        if (Character.isLetterOrDigit(c) || c == '_') {
            ...
            throw new IllegalArgumentException(
            		String.format(
            				"Illegal character \"%s\" at position %d is not allowed as a start of a name in a path template \"%s\".",
            				c, 
            				ci.pos(), 
            				template
            		)
        ...
        while (true) {
            ...
            if (Character.isLetterOrDigit(c) || c == '_' || c == '-' || c == '.') {
                ...
                if (c == ':') {
                    ...
                    throw new IllegalArgumentException(
                       String.format(
                		   "Illegal character \"%s\" at position %d is not allowed after a name in a path template \"%s\".",
                		   c, 
                		   ci.pos(), 
                		   template
                    	)
                ...
                throw new IllegalArgumentException(
                	String.format(
                		"Illegal character \"%s\" at position %d is not allowed as a part of a name in a path template \"%s\".",
                		c, 
                		ci.pos(), 
                		template
                	)
        ...
        try {
            ...
            throw new IllegalArgumentException(
            	String.format(
            		"Invalid syntax for the expression \"%s\" associated with the name \"%s\" in a path template \"%s\".",
            		nameRegexString, 
            		name, 
            		template), 
            	ex
    ...
    private char consumeWhiteSpace(final CharacterIterator ci) {
        ...
        do {
            c = ci.next();
        } while (Whitespace.isWhitespace(c));
